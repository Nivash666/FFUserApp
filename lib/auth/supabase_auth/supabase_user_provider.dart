...
class UserShoppingAppSupabaseUser extends BaseAuthUser {
  UserShoppingAppSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> userShoppingAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = UserShoppingAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
